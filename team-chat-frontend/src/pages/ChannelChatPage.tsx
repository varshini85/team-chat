...
const ChannelChatPage: React.FC = () => {
  ...
  // Initial load: channels + messages + membership + member count
  useEffect(() => {
    if (Number.isNaN(cid)) {
  ...
  // WebSocket: presence + optional live messages
  useEffect(() => {
    ...
    socket.onmessage = (event) => {
      try {
        ...
        if (data.type === "presence") {
          ...
          // skip if it's our own message; we already added it via HTTP
  ...
  // Polling for latest messages (keeps both browsers in sync)
  useEffect(() => {
    if (Number.isNaN(cid)) return;
    ...
    const refreshLatest = async () => {
      try {
        ...
        // don't set error here to avoid noisy UI
      }
    };
