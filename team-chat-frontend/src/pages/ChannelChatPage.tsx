...
const ChannelChatPage: React.FC = () => {
  ...
  useEffect(() => {
    if (Number.isNaN(cid)) { setError("Invalid channel id"); setLoading(false); return; }
    setError("");
    setLoading(true);
    const load = async () => {
    try {
...
setLoadingMore(true);
setError("");
try { const res = await api.get<Message[]>(`/messages/${cid}`, { params: { limit: PAGE_SIZE, offset }, });
...
setError(
err?.response?.data?.detail || err?.message || "Failed to join channel"
);
}
};
...
const onlineCount = onlineUserIds.length;
  ...
  return (
    ...
    <div> <Link to="/channels">← Back</Link> <span style={{ marginLeft: 8 }}> #{channel?.name ?? `Channel ${cid}`} </span> </div>
    ...
    {!isMember && (
    <div style={{ marginBottom: 8, padding: 8, borderRadius: 6, background: "#FEF3C7", border: "1px solid #FACC15",}} >
    <p style={{ margin: 0, marginBottom: 4 }}> You are not a member of this channel. </p>
    <button onClick={handleJoinChannel}>Join channel</button>
    </div>
    )}
    <div style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 180px)", border: "1px solid #ccc", borderRadius: 8, padding: 8,}}>
    ...
    {/* Load older button */}
    {!loading && hasMore && (
    <button onClick={handleLoadOlder} disabled={loadingMore} style={{ marginBottom: 8, padding: "4px 8px", fontSize: 12, cursor: loadingMore ? "default" : "pointer" }}>
    {loadingMore ? "Loading…" : "Load older messages"} </button>
    )}
    {!loading && messages.map((m) => { const senderName = (m as any).sender?.name ?? "Unknown"; let timeLabel = "";
    try { timeLabel = new Date(m.created_at).toLocaleTimeString([], {hour: "2-digit", minute: "2-digit"}); } 
    catch { timeLabel = "";}
    return ( <div key={m.id} style={{ marginBottom: 6 }}>
    <div style={{ fontSize: 11, color: "#555" }}>
    <strong>{senderName}</strong> {timeLabel}
    </div>
    <div style={{ display: "inline-block", padding: "4px 8px", borderRadius: 12, background: "#E5E7EB" }}>
    {m.text} </div> </div> );
    })}
    <div ref={endRef} /> </div>
    ...
    placeholder={isMember ? "Type a message..." : "Join the channel to send messages"}
    disabled={!isMember}/>
